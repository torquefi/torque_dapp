...
export default function BorrowForm({ availableBorrowOptions, onSubmit }: BorrowFormProps) {
  ...
  return (
    <div className="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-100 dark:border-gray-800 overflow-hidden">
        ...
        <div className="flex justify-between mb-2">
          {steps.map((step, index) => (
            <div key={step} className="flex flex-col items-center">
              <motion.div
                className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-medium transition-colors ${
                  index < currentStep
                    ...
                    : index === currentStep
                      ? "bg-[#F5EEFF] text-[#AA5BFF] border-2 border-[#AA5BFF]"
              ...
              <span className="text-xs mt-1 font-medium text-gray-500 dark:text-gray-400">{step}</span>
            </div>
          ))}
        </div>
        <div className="w-full bg-gray-100 dark:bg-gray-800 h-1 rounded-full mt-2 overflow-hidden">
          <motion.div
            className="bg-[#AA5BFF] h-full rounded-full origin-left"
            initial={false}
            animate={{
      ...
      {/* Step Content */}
      <div ref={contentRef} className="px-6 py-4 min-h-[420px] relative overflow-hidden">
        <AnimatePresence initial={false} custom={direction} mode="wait">
          ...
          >
            {currentStep === 0 && (
              <SelectCollateral
                availableOptions={availableBorrowOptions}
                selectedCollateral={formData.collateral}
                onSelect={(collateral) => setFormData((prev) => ({ ...prev, collateral }))}
              />
            )}
            {currentStep === 1 && (
              <SelectBorrowAsset
                ...
                onSelect={(borrowAsset) => setFormData((prev) => ({ ...prev, borrowAsset }))}
              />
            )}
            {currentStep === 2 && (
              <SelectAmount
                ...
                onChange={(collateralAmount, borrowAmount) =>
                  setFormData((prev) => ({ ...prev, collateralAmount, borrowAmount }))
                }
              />
            )}
            {currentStep === 3 && (
              <SelectLoop
                ...
                onChange={(loopCount) => setFormData((prev) => ({ ...prev, loopCount }))}
              />
            )}
            {currentStep === 4 && <ReviewLoan formData={formData} isSubmitting={isSubmitting} />}
          </motion.div>
        </AnimatePresence>
      </div>
      ...
      <div className="px-6 py-4 border-t border-gray-100 dark:border-gray-800 flex justify-between">
        <motion.button
          ...
          className={`px-5 py-2.5 rounded-full font-medium transition-colors ${
            currentStep === 0
              ? "bg-gray-50 text-gray-300 cursor-not-allowed dark:bg-gray-800 dark:text-gray-700"
        ...
        {currentStep < steps.length - 1 ? (
          <motion.button
            ...
            className={`px-5 py-2.5 rounded-full font-medium transition-colors ${
              isNextDisabled()
                ? "bg-[#AA5BFF]/50 text-white cursor-not-allowed"
          ...
          <motion.button
            ...
            className="px-5 py-2.5 rounded-full font-medium bg-[#AA5BFF] text-white hover:bg-[#9240E3] disabled:bg-[#AA5BFF]/50 flex items-center"
          >
            {isSubmitting ? (
              <>
                <svg
