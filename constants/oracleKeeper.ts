...
import { ARBITRUM, ARBITRUM_GOERLI, AVALANCHE, AVALANCHE_FUJI } from "./chain";

const ORACLE_KEEPER_URLS: any = {
...
export function getOracleKeeperRandomIndex(chainId: number, bannedIndexes?: number[]): number {
  ...
  if (bannedIndexes?.length) {
    const filteredUrls = urls.filter((url: any, i: any) => !bannedIndexes.includes(i));
